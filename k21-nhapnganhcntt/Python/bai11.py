...
class App(tk.Tk):
    def __init__(self):
        ...
        tk.Button(self, text='Nhap ban kinh', width=10, command=self.openNewWindow).pack(pady=12, padx=8)


    def openNewWindow(self):

        self.newOne = tk.Toplevel(self)
        self.newOne.title("Nhap ban kinh")

        tk.Label(self.newOne, text='Ban kinh').grid(row=0, column=1, padx=10, pady=10, sticky=tk.W)
        tk.Entry(self.newOne, width=13, textvariable=self.radius).grid(row=0, column=2, padx=8, pady=10, sticky=tk.W)
        tk.Button(self.newOne, text='Ve', width=10, command=self.drawCircle).grid(row=1, column=2, padx=8, pady=10)
